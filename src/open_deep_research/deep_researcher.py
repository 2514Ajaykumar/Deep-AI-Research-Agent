...
async def clarify_with_user(state: AgentState, config: RunnableConfig) -> Command[Literal["write_research_brief", "__end__"]]:
    ...
    model_config = {
        ...
        "max_tokens": configurable.research_model_max_tokens,
        "api_key": get_api_key_for_model("llama-3.3-70b-versatile", config),
    }
...
async def supervisor_tools(state: SupervisorState, config: RunnableConfig) -> Command[Literal["supervisor", "__end__"]]:
    ...
    if conduct_research_calls:
        try:
            ...
            if is_token_limit_exceeded(e, "llama-3.3-70b-versatile") or True:
...
async def compress_research(state: ResearcherState, config: RunnableConfig):
    ...
    configurable = Configuration.from_runnable_config(config)
    synthesizer_model = get_model().with_config({
        "model": "llama-3.3-70b-versatile",
    ...
    while synthesis_attempts < max_attempts:
        try:
            ...
            if is_token_limit_exceeded(e, "llama-3.3-70b-versatile"):
...
async def final_report_generation(state: AgentState, config: RunnableConfig):
    ...
    if notes:
        ...
        findings = "Explain the topic clearly and in detail."
...
Write a detailed, well-structured, clear answer.
"""
